...
> 기존 AWS 인프라를, 포트폴리오 열람 목적에 맞게 **월 비용이 0원에 수렴하는 구조로 전환**하기 위한 설계 기록.

> [!IMPORTANT]
> **현재 상태: Designed / Not Implemented**
> 아래 전환 후보 구조(Cloudflare R2, Queue, Worker, 무료 Runtime 등)는 **설계만 완료된 상태이며 실제 이전은 구현되지 않았다.**
> 이 문서는 재배포 자체보다 전환 설계 의도를 정리하는 데 목적이 있으며, 실제 이전 작업은 필요 시점에 진행한다.

---

## 1. 개요

이 문서는 "서비스를 다시 배포했다"는 결과 보고가 아니라, **기존 설계 의도를 유지하면서도 비용 제약 안에서 어떻게 다시 구성할지 판단한 과정**을 남긴 기록이다.

- 기존 AWS 무료 계정 종료 및 계정 삭제로 공개 배포가 중단되었다.
- 프로젝트를 폐기한 것은 아니며, 포트폴리오 열람 목적에 맞는 저비용 구조로의 전환안을 정리했다.
- 실제 상용 서비스 수준의 상시 인프라 비용이 아니라, **열람·데모 트래픽을 기준**으로 비용 최소화를 우선한다.
